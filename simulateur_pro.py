...
# 2. Initialisation des états de session pour suivre le parcours utilisateur
...
# 3. Récupération sécurisée de la clé d'API Gemini
try:
...
# 4. Fonctions globales de calcul et génération du rapport
...
def creer_pdf(texte_ia, age, patrimonio, epargne, rendement):
    ...
    donnees_table = [["Âge", f"{age} ans"], ["Patrimoine", f"{patlimonio:,.0f} €"], ["Épargne", f"{epargne} €/mois"], ["Rendement", f"{rendement} %"]]
    ...
    # PAGES 12 À 14 : ANNEXES FISCALES PEDAGOGIQUES
    ...
    # PAGE 15 : MENTIONS LEGALES & SIGNATURES
    story.append(PageBreak())
...
with col_inputs:
    ...
    if st.button("🧮 Calculer mes projections gratuitement"):
        ...
        st.session_state.pdf_pret = None # Force le recalcul si les chiffres changent
