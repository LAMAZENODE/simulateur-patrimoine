...
def creer_pdf(texte_ia, age, patrimoine, epargne, rendement):
    ...
    for lettre, titre in [("A", "L'Assurance-Vie"), ("B", "Le PEA"), ("C", "Le PER")]:
        ...
        story.append(Paragraph("L'optimisation globale implique l'usage de cette enveloppe pour capitaliser les intérêts sur le long terme à l'abri de l'impôt.", style_corps))
        story.append(Paragraph("Les prélèvements sociaux s'appliquent uniquement au moment des rachats partiels effectués par le souscripteur.", style_corps))

    # PAGE 15 : CLAUSES LÉGALES & SIGNATURES
    story.append(PageBreak())
